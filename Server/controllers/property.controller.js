...
export const updatePropertyStatus = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        error.message || "Internal server error while updating property status",
    });
  }
};
...
export const getPropertyDetails = async (req, res) => {
  try {
    ...
    // only increment the view if not seller but if he edit then increase the view count
    ...
    res.status(500).json({
      success: false,
      message:
        "Internal server error while fetching property details" ||
        error.message,
    });
  }
};
